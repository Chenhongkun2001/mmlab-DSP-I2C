...
CANIntNumberGet(uint32_t ui32Base, unsigned char ucNumber)
{
    ...
    case CANA_BASE:
    {
        ...
        case 0:
        {
            lIntNumber = INT_CANAINT1;
            break;
        }
        case 1:
        {
            lIntNumber = INT_CANAINT2;
            break;
    ...
    case CANB_BASE:
    {
        ...
        case 0:
        {
            lIntNumber = INT_CANBINT1;
            break;
        }
        case 1:
        {
            lIntNumber = INT_CANBINT2;
            break;
...
CANBitTimingSet(uint32_t ui32Base, tCANBitClkParms *pClkParms)
{
    ...
    uSavedInit = HWREG(ui32Base + CAN_O_CTL);
    HWREG(ui32Base + CAN_O_CTL) = uSavedInit | CAN_CTL_INIT | CAN_CTL_CCE;
    ...
    uBitReg |= (pClkParms->uQuantumPrescaler - 1) & CAN_BTR_BPR_M;
    uBitReg |= ((pClkParms->uQuantumPrescaler - 1) << 10)& CAN_BTR_BRPE_M;
    HWREG(ui32Base + CAN_O_BTR) = uBitReg;
    ...
    HWREG(ui32Base + CAN_O_CTL) = uSavedInit;
}
              ...
              uint32_t *pui32TxCount)
{
    ...
    ui32CANError = HWREG(ui32Base + CAN_O_ERRC);

    // Extract the error numbers from the register value.
    *pui32RxCount = (ui32CANError & CAN_ERRC_REC_M) >> CAN_ERRC_REC_S;
    *pui32TxCount = (ui32CANError & CAN_ERRC_TEC_M) >> CAN_ERRC_TEC_S;

    if(ui32CANError & CAN_ERRC_RP)
    {
        return(true);
    }
    return(false);
...
CANMessageClear(uint32_t ui32Base, uint32_t ui32ObjID)
{
    ...
    HWREGH(ui32Base + CAN_O_IF1CMD + 2) =
        (CAN_IF1CMD_DIR | CAN_IF1CMD_ARB) >> 16;
    HWREG(ui32Base + CAN_O_IF1ARB) = 0;
...
CANGlobalIntDisable(uint32_t ui32Base, uint32_t ui32IntFlags)
{
    ...
    //enable the requested interrupts
    HWREG(ui32Base + CAN_O_GLB_INT_EN) &= ~ui32IntFlags;
...
CANGlobalIntClear(uint32_t ui32Base, uint32_t ui32IntFlags)
{
    ...
    //enable the requested interrupts
    HWREG(ui32Base + CAN_O_GLB_INT_CLR) = ui32IntFlags;
...
CANGlobalIntstatusGet(uint32_t ui32Base, uint32_t ui32IntFlags)
{
    ...
    //enable the requested interrupts
    if( HWREG(ui32Base + CAN_O_GLB_INT_FLG) & ui32IntFlags)
        return true;
    else
        return false;
